...
async fn main() -> anyhow::Result<()> {
    ...
    // Ensure queues exist
    channel
        .queue_declare("dyno-submit-queue", QueueDeclareOptions::default(), FieldTable::default())
        .await?;
    channel
    ...
    channel
        ...
        .await?;
    channel
        .queue_declare("dyno-workflow-status-queue", QueueDeclareOptions::default(), FieldTable::default())
        .await?;
                    ...
                    Ok(delivery) => {
                        ...
                        match serde_json::from_slice::<Workflow>(payload) {
                            Ok(wf) => {
                                ...
                                if let Err(e) = register_and_publish_initial(&ch, &registry, wf).await {
...
async fn register_and_publish_initial(ch: &Channel, registry: &Registry, wf: Workflow) -> anyhow::Result<()> {
    // Build graph
...
async fn handle_status(ch: &Channel, registry: &Registry, status: TaskStatus) -> anyhow::Result<()> {
    ...
    let mut publish_list: Vec<TaskMessage> = Vec::new();
    {
        ...
        if status.status != "finished" {
            eprintln!("task {} reported non-finished: {}", status.task_id, status.status);
            return Ok(());
        }
        ...
        let &finished_idx = match state.node_map.get(&status.task_id) {
            Some(i) => i,
            None => {
                eprintln!("unknown task id {} for workflow {}", status.task_id, status.workflow_id);
                return Ok(());
            }
        };
        ...
        for child in state.graph.neighbors_directed(finished_idx, Direction::Outgoing) {
            if let Some(deg) = state.indegree.get_mut(&child) {
                ...
                *deg -= 1;
                ...
                eprintln!("missing indegree entry for child {:?}", state.graph[child].id);
            }
        }
